...
function onTweet(sender, result, more) {
  ...
  twitter.get('search/tweets', {
    ...
    if (messages.length < 1) {
      return messenger.sendMessage(sender, {
        text: 'Whoops, looks like I didn\'t get any Tweets back from Twitter. Sorry :('
      });
    }
...
function onLocation(sender, result, coordinates, title) {
  ...
  twitter.get('trends/closest', coordinates, (err, data, response) => {
    if (err) {
      return messenger.sendMessage(sender, {
        text: 'Sorry, there was an error talking to Twitter :('
      });
    }
    ...
    if (data && data[0] && data[0].woeid) {
      ...
      return messenger.sendMessage(sender, {
        text: 'Sorry, I couldn\'t associate your location :('
      });
    }
    ...
    messenger.sendMessage(sender, {
      text: hadLocation && name ?
        `I've changed your location to ${name}.` : 'Thanks for sharing your location :)'
    }, () => {
      if (client.get('wasAsking') === 'trending') {
        return onTrending(sender);
      }

      client.set('wasAsking', null);
      client.set('lastQuestion', 'trending');

      return messenger.sendMessage(sender, {
        text: 'Do you want to see trending topics around?',
        'quick_replies': [{
          'content_type': 'text',
          'title': 'Yes, please.',
          'payload': 'Yes'
...
function onYes(sender, result) {
  ...
  if (lastQuestion === 'name' && !interacted) {
    interacted = true;
  ...
  if (lastQuestion === 'location' && !interacted) {
    interacted = true;

    messenger.sendMessage(sender, {
      text: `OK, please tap the button to share your location. Alternatively you can also type it in here.`
    });
  }

  if (lastQuestion === 'help' && !interacted) {
    interacted = true;
  ...
  if (lastQuestion === 'change_location' && !interacted) {
    interacted = true;
  ...
  if (lastQuestion === 'trending' && !interacted) {
    interacted = true;
  ...
  if (wasAsking === 'trending' && !interacted) {
    interacted = true;
